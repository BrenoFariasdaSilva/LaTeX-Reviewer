...
def detect_unresolved_in_pdf(content, report, pdf_file):
    """
    Detect unresolved references in PDF content and append to report.
    ...
    if "??" in content:  # Detect unresolved references in rendered output
        report["unresolved_references"].append(  # Append occurrence to the report
            {
...
def detect_repeated_left_parentheses_in_pdf(content, report, pdf_file):
    """
    Detect repeated left parentheses in PDF content and append to report.
    ...
    if "((" in content:  # Detect repeated left parentheses in rendered output
        report["repeated_left_parentheses"].append(  # Append occurrence to the report
            {
                "file": pdf_file,  # The file path where the issue was found
                "line": None,  # No line information for PDF-level checks
                "column": None,  # No column information for PDF-level checks
...
def detect_repeated_right_parentheses_in_line(filepath, line, line_number, report):
    ...
    if "))" in line:  # If there are repeated closing parentheses in the line
        report["repeated_right_parentheses"].append(  # Add the repeated parentheses occurrence to the report with relevant details
            {
                "file": str(filepath),  # File path where issue was found
                "line": line_number,  # Line number where issue was found
                "column": line.find("))") + 1,  # Column index (1-based) of the match
...
def fix_duplicate_citations(filepath, line, line_number, report):
    ...
    for m in re.finditer(r"(\\cite[a-zA-Z]*\s*\{([^}]+)\})", line):  # Find all \cite-like commands on the line
        ...
        new_line, new_full, was_modified = build_new_citation_and_line(full, inner, line)  # Build replacement if duplicates present
        if not was_modified:  # Nothing to fix when 0 or 1 key or no duplicates
            continue  # Continue with next match

        append_duplicate_citation_report(report, filepath, line_number, full, new_full, str(line).strip())  # Record the duplicate citation fix
...
def main():
    ...
    print(
        f"{BackgroundColors.CLEAR_TERMINAL}{BackgroundColors.BOLD}{BackgroundColors.GREEN}Welcome to the {BackgroundColors.CYAN}Latex Reviewer Python Project{BackgroundColors.GREEN}!{Style.RESET_ALL}",
        end="\n\n",
